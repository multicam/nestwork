<script>
    let sitemap = [
        {
            title: 'Purchase a NestLove Property',
            links: [
                {
                    label: 'Properties Available Now',
                    url: '/#'
                },
                {
                    label: 'Frequently Asked Questions',
                    url: '/#'
                },
                {
                    label: 'Terms & Conditions',
                    url: '/#'
                }
            ]
        },
        {
            title: 'NestLove Calculator',
            links: [
                {
                    label: 'How to Use Video',
                    url: '/#'
                },
                {
                    label: 'Calculate Your Affordability',
                    url: '/#'
                },
                {
                    label: 'Calculate Your Potential ROI',
                    url: '/#'
                }
            ]
        },
        {
            title: 'Case Studies',
            links: [
                {
                    label: 'Happy NestLovers',
                    url: '/#'
                },
                {
                    label: 'Tax Incentives',
                    url: '/#'
                },
                {
                    label: 'Help and Support',
                    url: '/#'
                }
            ]
        },
        {
            title: 'NestLovers',
            links: [
                {
                    label: 'About Us',
                    url: '/#'
                },
                {
                    label: 'Meet the Founders',
                    url: '/#'
                },
                {
                    label: 'Contact us',
                    url: '/#'
                }
            ]
        },
        {
            title: 'References',
            links: [
                {
                    label: 'Privacy',
                    url: '/#'
                },
                {
                    label: 'Copyright',
                    url: '/#'
                },
            ]
        },
        {
            title: 'NestLove Community',
            links: [
                {
                    label: 'Social',
                    url: '/#'
                },
                {
                    label: 'NestLover Testimonials',
                    url: '/#'
                },
                {
                    label: 'Events',
                    url: '/#'
                }
            ]
        },
    ]
</script>
<footer id="footer">
    <div class=" row px blue pt8 mo-wrap">
        <h3 class="col-12 font-3_3vw mb4 mo-font-3_12vw mo-mb4">NestLove Makes Dreams… Become Home.</h3>
        {#each sitemap as cat }
            <div class="col-6 flex-col mb4 mo-mr4">
                <h5 class="uc mb1_5 mo-mb4 font-1_25vw">{cat.title}</h5>
                {#each cat.links as link }
                    <a class="text-white decoration-none  mb0_5 mo-mb1 book  " href="{link.url}">{link.label}</a>
                {/each}
            </div>
        {/each}
    </div>
    <div class="row blue px pb4">
        <div class="col-9 flex-col"><h4 class="font-2_5vw">
            <a class="text-yellow decoration-none hover-white" href="https://nestlove.com.au/login">NestLover Login</a>
        </h4>
        </div>
        <div class="social off-1 col-2 text-right mt0_5 mo-w-full mo-text-left">
            <a aria-label="Instagram"
               class="text-white decoration-none  mb0_5 mo-mb1 book   ml2 mo-ml2 text-yellow hover-white"
               href="https://www.instagram.com/nestloveproperty/" rel="noopener" target="_blank">
                <svg class="instagram" id="Instagram" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10,1.778a30.662,30.662,0,0,1,4,.111,5.154,5.154,0,0,1,1.889.333,3.9,3.9,0,0,1,1.889,1.889A5.154,5.154,0,0,1,18.111,6c0,1,.111,1.333.111,4a30.662,30.662,0,0,1-.111,4,5.154,5.154,0,0,1-.333,1.889,3.9,3.9,0,0,1-1.889,1.889A5.154,5.154,0,0,1,14,18.111c-1,0-1.333.111-4,.111a30.662,30.662,0,0,1-4-.111,5.154,5.154,0,0,1-1.889-.333,3.9,3.9,0,0,1-1.889-1.889A5.154,5.154,0,0,1,1.889,14c0-1-.111-1.333-.111-4a30.662,30.662,0,0,1,.111-4,5.154,5.154,0,0,1,.333-1.889A3.991,3.991,0,0,1,3,3a1.879,1.879,0,0,1,1.111-.778A5.154,5.154,0,0,1,6,1.889a30.662,30.662,0,0,1,4-.111M10,0A32.83,32.83,0,0,0,5.889.111,6.86,6.86,0,0,0,3.444.556,4.35,4.35,0,0,0,1.667,1.667,4.35,4.35,0,0,0,.556,3.444,5.063,5.063,0,0,0,.111,5.889,32.83,32.83,0,0,0,0,10a32.83,32.83,0,0,0,.111,4.111,6.86,6.86,0,0,0,.444,2.444,4.35,4.35,0,0,0,1.111,1.778,4.35,4.35,0,0,0,1.778,1.111,6.86,6.86,0,0,0,2.444.444A32.83,32.83,0,0,0,10,20a32.83,32.83,0,0,0,4.111-.111,6.86,6.86,0,0,0,2.444-.444,4.662,4.662,0,0,0,2.889-2.889,6.86,6.86,0,0,0,.444-2.444C19.889,13,20,12.667,20,10a32.83,32.83,0,0,0-.111-4.111,6.86,6.86,0,0,0-.444-2.444,4.35,4.35,0,0,0-1.111-1.778A4.35,4.35,0,0,0,16.556.556,6.86,6.86,0,0,0,14.111.111,32.83,32.83,0,0,0,10,0m0,4.889A5.029,5.029,0,0,0,4.889,10,5.111,5.111,0,1,0,10,4.889m0,8.444A3.274,3.274,0,0,1,6.667,10,3.274,3.274,0,0,1,10,6.667,3.274,3.274,0,0,1,13.333,10,3.274,3.274,0,0,1,10,13.333m5.333-9.889a1.222,1.222,0,1,0,1.222,1.222,1.233,1.233,0,0,0-1.222-1.222"
                          fill="currentColor"></path>
                </svg>
            </a>
        </div>
    </div>
    <div class="row blue px pb4">
        <div class="col-12">
            <small>Copyright NestLove &copy; {new Date().getFullYear()}</small>
        </div>
    </div>
</footer>

<style lang="scss">
    footer {
        background-color: var(--bg-color);
        color: snow;

        a {
          font-weight: 500;
          text-decoration: none;
          color: rgba(255, 250, 250, 0.8);
          &:hover {
            color: rgba(159, 59, 46, 0.9) !important;
          }
        }
    }
    .social svg {
        height: 2vw
    }
    small {
        align-self: flex-end;
    }
</style>
